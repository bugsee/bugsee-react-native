...
The schema lives in `bugsee/specs` at `sdk/options/`. That repository was renamed from `bugsee/report-bundle-structure` and restructured to hold specs from several areas, not only the report bundle. Specified in `bugsee/specs#14`. Every wrapper consumes it, and it can also drive documentation generation, `wizard-cli`, and the AI skills.
...
5. **Open issues filed while designing:**
   ...
   - `bugsee/bugsee-cocoa#88` — the React Native log hook was duplicated between the public `BugseeLogger` and the live `BGSLoggerInterceptor`, with the level mapping in both. PR #89 consolidates it.
     ...
     **Superseded in part.** That issue asked *where* the hook should live. The answer turned out to be *nowhere*: see §10.3. `bugsee/bugsee-cocoa#91` and `bugsee/bugsee-android#90` add a generic source-aware log API to each SDK, after which the React-Native-specific code is deleted from the Apple core outright and this wrapper owns the `RCTLogLevel` mapping. #89 was repurposed accordingly: it consolidates the CocoaLumberjack mapping and removes the React-Native-specific code outright, rather than adding public API that would be removed a release later.

6. **Improves §10.3, does not block it:** `bugsee/bugsee-cocoa#91` and `bugsee/bugsee-android#90`. Once #89 removes the iOS hook, the wrapper forwards logs through the public `log:level:enforceFiltering:` API on both platforms, so the filter runs and the two platforms agree — both recording `source: 4` (`Bugsee`), which is what Android already produces today. That is the divergence fixed. These two issues add the source-aware API that lets both platforms record `Custom` instead, which is strictly better but not a prerequisite.
